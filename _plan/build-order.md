...
measure 입력 재현은 범위 밖). delete는 note와 textEvent를 각각 별도
dispatch로 처리한다(한 undo로 합치지 않음, 결정 필요 항목). paste는
D-2026-123이 `pasteNotesAndTextEventsCommand`로 한 undo에 합쳐 닫았다.
...
**M6-4 진행 상황**(D-2026-110): 109건 전체 `DECISION_LOG` 라인별 재감사
대신, 위험도가 높은 세 갈래로 좁혀 실행했다 — (1) spec 문서에 남은
`결정 필요 항목` 표기 6곳 개별 확인, (2) `Defined in`이 코드 파일만
가리키는 결정(다단 라인까지 정확히 파싱해 10건으로 좁힘) 각각의 spec
반영 여부, (3) `[수정]` 태그가 붙은 결정 7건의 대상 spec이 실제로
수정된 값을 담고 있는지. 실제 drift 2건을 찾아 고쳤다: `src/app/README.md`
의 M5-1 단락이 "`.cfx`는 binary라 못 연다"고 남아 있었는데 M5-8/
D-2026-106이 이미 binary read를 닫아 stale이었다(주석 보강으로 해결).
`scene/scene.md` §6이 `song-credit → gameplay`의 `replace` 관례만
명시하고 D-2026-088이 `gameplay → result`로 확장한 것(스펙이 정하지
않은 이 세션의 결정으로 당시 보고됨)은 spec에 전혀 안 실려 있었다 —
`[수정]` 문단으로 추가했다. 나머지 확인 대상은 전부 이미 정확했다(6곳
중 5곳은 여전히 정당하게 열려 있는 잔여 항목, 10건 중 나머지는 spec을
바꿀 필요가 없는 "기존 spec 그대로 구현"뿐이었던 결정, `[수정]` 7건
중 나머지 5건은 이미 `Defined in`에 올바른 spec 파일이 있었다). 좁힌
스코프 밖(M1~M5의 settled 결정 전부)은 건드리지 않았다.
...
잔여:
- [x] 수동 대조 시나리오의 구체 목록 — 각 milestone 진입 시 작성 — **닫힘**(M6-4 재확인, D-2026-111): 별도의 사전 문서로는 안 만들었지만, 목적("무엇을 대조할지 구체적으로 정해 둔다")은 각 milestone의 `**Exit**:` 줄이 그 자리에서 실제로 채웠다 — M2(§5, 8개 named 시나리오: 전 SYNC/전 MISS/hard 사망/cascade 강등/Hold 동시 소유/mid-start/pause Resume/곡 끝 tail), M4(§7, "gameplay 구간에 한정"으로 M2 목록을 재사용·범위 제한), M5(§8, "편집 조작별 결과 비교" — 원본 대응물이 없는 에디터라 M5-8 보고의 New Chart→편집→저장→패키징→import→플레이 다단계 경로가 실질적 concrete list 역할을 했다). M3은 end-to-end 흐름 자체가 하나의 시나리오 문장이라 별도 목록이 필요 없었다. "진입 시 작성"이 아니라 "Exit에 명시"로 실현된 것은 실제 실행 방식의 차이일 뿐 항목의 의도는 충족됐다.
