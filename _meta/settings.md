...
- 키 배치(`DEFAULT_LANE_KEYS`/`DEFAULT_ACTION_KEYS`)도 PLAY 소속(상세 [[naming]] §3).
...
- **`gaugeMode`** — `Normal`|`Hard`|`AS`|`AP`|`FC`|`Cascade` 단일 축. 정의·강등·terminate 전체는 → [[gauge]]. (코드의 `gaugeType×lockTarget×lockMode` 직교를 6종 사다리로 평탄화, [[rationale]].)
