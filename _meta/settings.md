...
- **`judgeLinePos`** — 판정선 세로위치(필드 높이 분수). **기본 8/9 = 최하단**, 값이 작아질수록 위로 올라간다(**raise-only**, 기본 아래로는 못 내림). 올리면 게이지(판정선 겸)·콤보 블록·하단 곡정보 띠가 **함께 올라간다** `[수정 — 구는 띠가 기본 밴드 고정]`. 띠 렌더 세부는 [[theme]], 근거 → [[rationale#판정선 raise 시 곡정보 띠도 따라 올리는 이유]].
...
플레이어 settings와 별개로, **에디터에서만** 쓰는 1회 보조값. 곡과 함께 저장·교환되지 않는다. **UI 노출은 에디터 meta 씬의 '에디터 설정' 구역뿐** `[신규]` — 게임 settings 씬에는 나타나지 않는다([[editor-graph]] §4). (gridDivisor·laneGridDivisor는 세션 중 툴바 직접 조작인 editorState — 여기 아님.)
