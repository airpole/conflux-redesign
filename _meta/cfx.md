...
```
music.<ext>     ← 오디오 (필수, 정확히 1개)
jacket.<ext>    ← 자켓 (0~1개)
*.json          ← chart 파일 × N (§2 그대로)
```

- 에셋은 **고정명** `[번복]`: content-hash 폐기(dedup할 스토어가 없다). 확장자는 원본 유지, 오디오 포맷 화이트리스트 없음 — env의 디코딩에 위임, 실패는 에러 + 에디터에선 music 수동 재지정 허용. 자켓 인라인 금지는 유지.
...
- [x] 에셋 고정명 music/jacket, content-hash 폐기 `[번복]`, 포맷 디코딩 위임
