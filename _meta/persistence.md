...
- game-public: bundled curated `.cfx`만 — **확정** (D-2026-059, D-2026-021 해소). 라이브 웹 배포(관리자 업로드 → 접속 유저 플레이)는 도입하지 않는다.
- `.cfx`는 **평문**이다 — 암호화 계층을 두지 않는다(D-2026-059). chart·음원 파일은 누구든 완전히 추출 가능하다는 트레이드오프를 명시적으로 수용한 결과다.
- 서버 기반 기록은 여전히 D-2026-019로 보류한다 — 이번 결정은 공개 서비스 기록도 로컬 유지로 확인했을 뿐 그 판을 열거나 닫지 않는다.
- 라이선스 서드파티 트랙의 추출 가능성(계약 위반은 아니나 평판 리스크)은 D-2026-060으로 별도 팔로업 기록 — 지금은 조치하지 않는다.
...
**다운그레이드 포함 reimport는 confirm 후 허용한다** (D-2026-018). reimport는 songId blob 전체 교체이며 chart 단위 부분 병합은 없다. confirm UI가 위 비교(추가·삭제·upgrade·downgrade)를 표시하고, 사용자가 진행을 확정하면 다운그레이드가 포함되어도 교체한다. 자동 overwrite는 하지 않는다. records는 identity 기반이라 blob 교체와 무관하게 유지된다([[records]] §1).
...
기록은 chart identity를 따라 유지되며 내용 변경을 판별하지 않는다. content fingerprint는 도입하지 않는다([[records]] §1, D-2026-017).
...
- [x] 다운그레이드 포함 reimport confirm 후 허용 (D-2026-018)
- [x] records 내용 변경 무판별·identity 유지 (D-2026-017, [[records]])
