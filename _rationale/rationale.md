...
## cascade result 막대를 최종 티어 기준으로 바꾼 이유
[번복] — 구안은 "result 막대는 항상 hard 값"이었다. 근거였던 "lock 3종은 게이지가 무의미"는 lock 생존 종결에 100 고정 + state 색 규칙이 §2에 이미 있어 성립하지 않았고, 치명적으로 normal 단계에서 C로 종결한 판은 곡 끝 화면의 막대(normal 값·색)가 result에서 0으로 바뀌는 불연속이 생기며, normal 단일 모드의 result(normal 값 표시)와도 어긋나 "cascade로 낸 결과 = 그 단일 모드로 낸 결과"라는 동치 원칙이 표시 층위에서 깨졌다. 최종 생존 티어 기준으로 바꾸면 플레이 중 표시·result·단일 모드 표시 셋이 전부 이어진다.

## gauge를 2값 병렬 + tier 단일 구조로 통일한 이유
cascade가 요구하는 상태(게이지 2종 병렬 + 현재 생존 티어)를 담는 방법은 둘이었다: 기존 단수 게이지를 보존하고 cascade 전용 필드를 얹거나, 전 모드를 `gauge = {hardPct, normalPct}` + `tier` 공통 구조로 통일하거나. 후자를 택했다 — cascade가 "단일 모드들의 병렬 평가"라는 개념이 스키마에 그대로 드러나고, 게이지 누적 코드 경로가 하나가 되며(모드는 어느 값을 소비할지만 다름), 강등이 비가역이라 tier 하나가 래칫까지 표현한다. 단일 모드에서 소비하지 않는 쪽 게이지가 같이 구르는 비용은 무시 가능하고, 오히려 분기가 없어 단순하다. 키명은 명명 원칙(단위 접미사)에 따라 `hardPct`/`normalPct`. 한편 `playState.state`는 두지 않는다 — state는 `computeResult`가 반환하는 산출물이라는 data-model §9 원칙을 유지하고(저장하면 판정마다 갱신해야 하는 이중 상태), naming에 등록됐던 행은 `tier`로 교체했다.

## 중간 시작 시 게이지를 시드하는 이유
[수정] — 실측상 구 `seedPlayStateFromCurMs`는 hits(SYNC)·combo만 시드하고 게이지는 시작값 그대로 뒀다. 그 결과 normal 게이지로 곡 후반에서 시작한 판은 "앞을 전부 SYNC로 친 것"으로 취급되면서 게이지만 0이라, 남은 노트를 전부 SYNC로 쳐도 75%에 못 미칠 수 있는 자기모순 상태였다 — FC/AP 유효성은 시드로 보존하면서 게이지 유효성만 버리는 비대칭. 시드된 SYNC에 게이지 델타를 그대로 적용해 대칭을 회복한다: normal은 시드 유닛 수 × a 누적(상한 100), hard는 + 델타가 상한 클램프에 막혀 "100 유지"와 수학적으로 동치라, 규칙이 "시드 = SYNC 델타 적용" 하나로 통일된다. 중간 시작은 무기록 게이트에 걸리므로 기록·밸런스 영향은 없고, 화면 표시와 연습 체감의 정합이 목적이다.
