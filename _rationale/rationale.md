...
### sub 격자 통일의 대가와 처분 (D-2026-045)

- 트레이드오프 사실: sub를 gridDivisor 격자로 통일한 대가로, 박자표 전환점이 격자에 어긋난 chart에서 마디 상대 나머지가 cell로 나눠떨어지지 않아 **왕복 가능한 tick 집합이 좁아졌다**. 재현: TS `[{0,3,8},{2880,4,1}]`, gd 4에서 tick 238080 → "11.3.3" → 239040 (+960). 원본의 박 내부 16분할은 이 자리에서 왕복이 성립했다.
- 처분: 근사 표기를 버리고 `t` 폴백 — 왕복이 표기 형태와 무관하게 항상 성립하게 됐다. (a) 절대 격자 기준 sub는 읽기 규칙 변경이 에디터 UI까지 파급돼 보류, (b) 원본 복귀는 TM-7 취지 상실.
- 장기 방향 (소유자 결정 원문 요지): 절대 tick이 canonical이고 `bar.beat.sub`는 derived display representation이다. 표기→tick을 canonical round-trip의 전제로 삼지 않는다. 박자표 변경·grid/snap·편집·판정·serialization이 서로 독립 개념으로 정리되는, DAW 유사의 시간 모델을 지향한다. — 시간 모델 재정리 시점(에디터 표기 UI 설계, M2 전후)에 재론.
...
### 왜 골든 shape 표를 통째로 다시 뽑았나

이전 표 58건은 픽스처가 원본에 없는 필드를 쓰고 있었다. 이벤트에 `isBlue`·`targetPos` 대신 `blue: [10,20,30,40]` 같은 배열이 들어 있어서, 원본은 두 체인을 **비었다고 판정하고** 기본 기하만 돌려줬다. `getShape` 아홉 건이 전부 `{left: 32, right: null}`이었다.

easing도 같았다. 추출기가 `In`/`Out`/`InOut`을 넘겼는데 원본의 실제 가지는 `Linear`/`In-Sine`/`Out-Sine`/`Arc`다. 목록에 없는 이름은 예외 없이 Linear로 떨어지므로 28건이 전부 같은 값이 됐다 — **In-Sine·Out-Sine은 한 번도 측정된 적이 없었다.**

하네스의 빈 표 방어가 걸리지 않은 것은 그 검사가 "전부 비었는가"를 보기 때문이다. 절반만 비면 통과한다. 표가 있다는 사실이 대조가 있다는 뜻은 아니라는 것 — 이것이 이 milestone에서 가장 값비싼 교훈이다.

표본 tick에 보간 **도중** 지점을 넣은 것도 같은 이유다. 끝점만 재면 어떤 곡선을 써도 값이 같아서, easing이 걸리는지 확인할 수 없다.

### 왜 anchor를 "시작값 하나"로 두나

`shape` §4는 두 말을 함께 담고 있었다. 글머리는 anchor가 "그 tick에 값을 못박는다"고 했고, 바로 아래 평가 절차는 anchor를 시작값으로만 쓰고 나머지를 버렸다. 원본을 돌려 보니 후자다 — 중간 anchor는 체인에서 걸러지고 **아무 일도 하지 않는다.**

못박는 쪽이 더 균일해 보이지만 예외가 하나 생긴다. 첫 anchor의 값은 곡 시작 전(음수 tick)에도 유효해야 하는데, anchor가 자기 tick에서만 값을 세운다면 pre-roll 구간의 모양이 tick 0에서 튄다. "첫 anchor만 소급 적용"이라는 특례를 따로 둬야 균일함이 유지된다.

시작값 하나로 두면 그 특례가 필요 없다. anchor에는 tick이 의미를 갖지 않고, 값은 모든 tick에서 정의된다. 대신 "체인당 anchor 하나"라는 조건이 생기는데, 이것은 `validateChartDomain`이 보고한다 — 둘째 anchor는 화면에 아무 흔적 없이 사라지는 데이터라 조용히 두면 안 된다.

데이터 모델에서 anchor를 아예 빼고 체인마다 `initPos` 값 하나를 두는 쪽이 더 깨끗하다(그러면 "anchor 하나"가 규칙이 아니라 **불가능한 상태**가 된다). 저장 형식이 바뀌어 `cfx`·`data-model`까지 열어야 하므로 이번 범위에 넣지 않았다.

### 왜 anchor 선택만 원본과 다르게 했나

원본은 anchor가 여럿일 때 **배열에 먼저 적힌** 것을 쓴다. 이것은 계승하지 않았다(SH-6).

배열 순서에 기대면 같은 chart를 다시 저장하는 것만으로 모양이 바뀔 수 있다. 편집·정렬·직렬화 중 어느 하나라도 순서를 건드리면 시작 위치가 조용히 옮겨간다. `chart.notes`의 배열 순서는 배치 순서라는 뜻이 있어 보존하지만, anchor 선택은 그런 뜻이 없는 자리에서 순서에 의미를 부여한 것뿐이다.

### 왜 같은 tick 정렬 규칙을 명문화했나

`shape` §4는 "시간순 정렬"까지만 말하고 같은 tick끼리의 순서를 말하지 않았다. 원본에는 규칙이 있고(`duration 0`이 먼저), **그 순서가 값을 바꾼다** — tick T에 즉시 점프(→32)와 보간(→64)이 함께 있으면 점프가 먼저여야 보간이 32에서 출발한다. 안 정하면 정렬이 배열 순서에 기대게 되어 같은 chart가 다른 모양을 낸다.

### 왜 모르는 easing을 거부하지 않나

원본은 목록 밖 easing을 조용히 Linear로 떨어뜨린다. 거부하면 옛 차트가 열리지 않고, 편집 중 값이 잠깐 이상해질 때마다 막힌다 — `data-model` §11이 검증을 두 층으로 가른 이유가 그대로 여기 걸린다.

그래서 모양은 원본과 같게 두고 domain 검증이 **보고**한다. 조용한 오작동을 보이는 오작동으로 바꾸는 것이 이 층의 몫이다.

### 왜 shape와 lane을 한 파일에 두나

`shape` §4가 체인 평가의 단일 출처이고 `lane-events` §6이 그 절을 참조한다. 두 벌은 선택자(`isBlue` ↔ `lineNum`)와 좌표계만 다르고 평가는 글자 그대로 같다. 파일을 가르면 얇은 파일 하나가 생기고, 문서가 "여기가 단일 출처"라고 말한 구조를 코드가 다시 흩는다.

### 표를 고치는 조건

M1-4는 상수 이름이, M1-5는 상태 필드 이름이 `naming`을 이탈해 있었고 두 번 다 **구현을 표에 맞췄다.** M1-9는 반대로 갔다(`judgeKeyDown`/`judgeKeyUp`, `closeTail`). 두 사례를 가르는 것은 다음 한 가지다.

> **표를 고치는 것은 구현이 표보다 뒤에 알게 된 사실을 담고 있을 때뿐이다.** 구현이 표를 보지 않아서 생긴 차이는 언제나 구현을 고친다.

M1-4의 `WINDOW_*`는 M1-2가 대응표를 대조하지 않고 원본 이름을 그대로 쓴 것이었다 — 새로 알게 된 것이 없다. M1-9의 두 건은 다르다. `judgeKeyPress`는 `judgeKeyUp`이 없던 때 지은 이름이고, `commitTailRelease`/`commitMidRelease`는 M1-5에서 두 경우가 **판정 종류만 다른 같은 계산**임이 드러나 한 함수가 됐다. 이름을 둘로 되돌리면 같은 계산이 두 벌이 된다.

판별법은 "표를 고칠 이유를 그 자리에서 새로 알게 된 사실 한 문장으로 적을 수 있는가"다. 적을 수 없으면 구현이 이탈한 것이다. 이 조건을 적어 두지 않으면 다음 이탈 때 이 선례가 변명으로 쓰인다.

새로 알게 된 사실 문장은 측정 가능한 출처(원본 파일·줄, 골든 케이스, 테스트)를 인용해야 한다. 인용할 출처가 없으면 그것은 새로 알게 된 사실이 아니다.
이 조건은 어긋남의 **처분**만 정한다 — 어긋남의 **발견**은 별도 장치(골든·명칭 가드·뮤테이션)의 몫이며, 표와 구현이 일치한 채 둘 다 무검증인 상태는 이 조건이 발동조차 하지 않는다.
