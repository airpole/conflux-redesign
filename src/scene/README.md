...
Ctrl+D 복제 모두 `deleteNotesAndTextEventsCommand`(D-2026-124)·
`pasteNotesAndTextEventsCommand`(D-2026-123, D-2026-125가 복제에도
재사용)로 note·textEvent를 한 undo에 합쳐 낸다 — Ctrl+Z 한 번에 하나만
되돌아가고 다른 하나는 남는 문제를 셋 다 닫았다. 재생 시 fade 표시는
이미 M4.5-1이 구현해 뒀다 — `scene-gameplay.ts`
