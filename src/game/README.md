...
**M5-6(부분, D-2026-103)이 `game-engine.ts`/`game-session.ts`에 mid-start를
더했다** — `startEngineSession`이 `startChartMs`(기본 0)·`leadInMs`(기본
`LEAD_IN_MS`)를 받아 0이 아닌 위치에서 세션을 연다. 시계는 anchor 이전에도
계속 흐르되(tick-0 lead-in과 같은 이유, 노트 스크롤-in 연출 유지) 새 `leadIn`
phase 동안 `paused`가 `true`라 호출측이 `registerKeyDown`/`registerKeyUp`만
쓴다([[judge]] §10). `leadInMs=0`은 test scene의 즉시재생(lead-in 없음,
