...
async function boot(root: HTMLElement, storage: StorageEnv): Promise<void> {
  ...
  // ── M6-2: editor scene graph — 동적 import로 옮겼다(`app-editor.ts`,
  // `_plan/architecture.md` §4). `FEATURES.editor`가 false인 public
  // 빌드에서는 이 블록 전체가 상수로 접혀 죽은 코드가 되고, 그 안의
  // `import()`가 Rollup에 결코 도달 가능한 것으로 보이지 않아
  // `app-editor.ts`와 그 정적 import 그래프(모든 editor scene·edit·
  // format-cfx-* 모듈)가 산출물에서 통째로 빠진다. editor scene들은
  // `createSceneManager([...])`를 만들기 전에 이미 완성돼 있어야 한다
  // (`Scene.mount()`는 동기라 늦게 끼워 넣을 방법이 없다) — 그래서 여기서
  // await한다.
  let editorScenes: readonly Scene[] = [];
  if (FEATURES.editor) {
    const { mountEditorScenes } = await import('./app-editor.js');
    const { editorStartScene, editorWorkspaceScenes } = mountEditorScenes({
      root,
      storage,
      audioEnv,
      gotoScene: (id) => manager.goScene(id),
      setPendingGameplayInput: (input) => {
        pendingGameplayInput = input;
      },
    });
    editorScenes = [editorStartScene, ...editorWorkspaceScenes];
  }
...
if (root !== null && typeof indexedDB !== 'undefined') {
  void boot(root, createStorageEnv(createIndexedDbBackend(indexedDB)));
}
