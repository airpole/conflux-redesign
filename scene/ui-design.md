...
### 2.5.8 quick options overlay (M4.6)

M3.5-1의 원래 범위 문구에 이 오버레이가 있었지만 §2.5.1~§2.5.7 어디에도
실제로 반영되지 않은 채 넘어갔다(D-2026-072의 유보 목록에도 없다 — 유보가
아니라 누락이었다). M4-7이 이 공백을 최소 기능 placeholder(설정 화면과
같은 토큰의 중앙 모달, 목록형 5행, 클릭은 "그 row를 고른다"는 역할만)로
메워 뒀고, 여기서 정식 레이아웃으로 대체한다(D-2026-093, M4.6-1). 필드
5종(scrollSpeed·gaugeMode·mirror·staticShape·autoplay)의 순서·확정 규칙
자체는 `scene.md` §5·§10이 이미 정한 것으로 재검토 대상이 아니다 — 여기는
그게 화면에 어떻게 배치·표현되는지만 다룬다.

**배치**: 중앙 정렬 dimmed modal. `scene.md` §10 "열림 중 scene 입력 차단"을
시각적으로도 뒷받침하도록 스크림(`rgb(5 5 8 / 70%)`, M4-7 placeholder와
동일 값 유지)을 그대로 쓴다 — §2.5.4 정보 패널 같은 도킹 패널이 아니라
전체 화면을 덮는 모달로 확정한다. 모달 자체의 폭·행 간격은 §2.6.3(settings
필드 표현 어휘)의 `.field-row`/`.field-label`/`.field-control` 치수를
그대로 재사용한다 — 새 치수 체계를 만들지 않는다.

**필드별 컴포넌트 — §2.6.3 어휘 재사용, 새 위젯 없음**:

- **scrollSpeed** — 네이티브 `<input type="range">`(§2.6.3의 slider 위젯과
  동일 컴포넌트, `SCROLL_SPEED_MIN`/`MAX`/`STEP`). 클릭이 그 위치의 값으로
  즉시 점프한다(브라우저 기본 동작) — [[scene]] §5 "마우스 클릭(그 값으로
  즉시 점프)"를 문자 그대로 만족한다.
- **gaugeMode** — segmented control(§2.6.3의 select 위젯, `.segment-group`/
  `.segment-btn` 그대로). 6개 모드(`GAUGE_MODES`)를 세그먼트로 나열해
  각각이 독립 클릭 타겟이다 — 클릭 = 그 모드로 즉시 점프.
- **mirror·staticShape·autoplay** — 이미 M4-7이 구현한 toggle-switch 클릭
  토글을 그대로 유지한다(§2.6.3의 toggle 위젯과 동일 컴포넌트) — bool
  필드는 애초에 두 값뿐이라 클릭 토글이 이미 "즉시 점프"를 만족했고
  재검토 대상이 아니다.

**활성 row 표시**: 풀 row outline(M4-7 placeholder의 `.quick-options-row.active`
테두리) 대신 좌측 강조 바 하나로 바꾼다 — settings 화면의 key-rebind
capturing 상태(§2.6.3, `--cyan` 좌측/테두리 강조)와 같은 시각 어휘다. 이
강조는 순수 표시고, ↑/↓로 이동한 row가 곧 다음 조작 대상이라는 걸
보여주는 것 외에 클릭 컴포넌트들의 동작을 바꾸지 않는다.

**닫을 때(Esc/Space)의 동작 — D-2026-092 수정**: M4-7은 오버레이를 닫을 때
확정 안 된 draft를 버렸다(row 이동과 같은 규칙을 닫기에도 확장한
잠정 결정, D-2026-092). 이 라운드에서 뒤집는다 — **닫기는 지금 활성
row의 draft를 암묵적으로 확정한다**(Enter를 누른 것과 동일). 근거:
클릭이 이제 실제 값 설정 동작이라(위 필드별 컴포넌트 항목), 클릭한
직후 Space로 나가면서 그 값이 사라지는 건 "빠른 조작"이라는 이 화면의
존재 이유와 반대로 느껴진다 — 되돌릴 안전장치보다 방금 고른 값이
그대로 남는 쪽이 이 표면의 기대에 맞는다. row 이동(↑/↓) 시 미확정
draft를 버리는 규칙은 그대로 유지한다 — "조작 중간에 다른 필드로
옮기면 그 필드는 미완성으로 남는다"는 것과 "떠난다"는 건 다른
액션이라 이 둘을 갈랐다.

**새 토큰 없음**: 스크림 색·모달 배경·필드 위젯 색 전부 §1(토큰)·§2.6.3
(field 위젯)이 이미 가진 값의 재사용이다.
...
## 2.9 mode-select 레이아웃 (M4-2 선행)

디자인 리뷰로 확정된 방향이다 — 아래는 재설계 대상이 아니라 그 결과의
문서화다. M3.5의 네 화면(song-select·settings·title·credits) 목록에
mode-select가 원래 없었다 — M4-2 구현 직전에 이 공백을 발견해 별도로
리뷰했다. 행동 규칙(항목 4개·목적지·`FEATURES.editor` 가시성·Back 키)의
단일 출처는 여전히 [[scene]] §4이며, 여기는 시각 표현만 다룬다.

스테이지는 §2/§2.5~§2.8과 같은 16:9 + `container-type: size` + `cqw`
단위를 재사용한다.

### 2.9.1 구조 — 세로 목록, 확장 가능한 형태
...
- 세로로 쌓은 목록 하나. [[scene]] §4의 "mode 추가의 단일 확장점"을
  반영해 **정확히 4칸으로 고정된 그리드가 아니라 항목이 늘어나도
  그대로 늘어나는 리스트**로 그린다 — 나중에 다섯 번째 mode가 추가돼도
  레이아웃 구조 자체를 다시 설계할 필요가 없다.
- 항목 라벨(`Play`/`Editor`/`Settings`/`Credits`)은 공용 영어
  (D-2026-077) — nav 라벨류라 `translate()`를 거치지 않는다.
- 활성/hover 항목은 `--cyan`, 나머지는 `--text` — §2.6 nav 바·§2.5 active
  탭과 같은 언어.
- 커서(키보드) 이동과 클릭 둘 다 선택 수단이다 — 다른 목록형 화면
  (§2.5 song-select)과 같은 원칙, 새 규칙 아님.

### 2.9.2 `Editor` 항목 가시성 — reflow, gap 없음

`FEATURES.editor`가 꺼지면 `Editor` 항목은 **완전히 빠지고 나머지 3개가
그 자리를 채우며 다시 배치**된다 — 숨겨진 자리 표시를 위한 빈 칸을
남기지 않는다. [[scene]] §8 "off면 button 미표시"가 이미 "표시 안 함"을
말하고 있고, `public` 빌드는 그 코드 자체가 번들에 없다(§8 "코드는
제거하지 않으며" — 이건 소스 레벨 얘기지 런타임에 목록에 항목으로
잡혀 있다는 뜻이 아니다) — 빈 자리가 남으면 "원래 뭔가 있었는데 없다"는
인상을 줘 오히려 그 존재를 암시한다. `Credits`는 반대로 항상 노출이라
([[scene]] §4 "항상 노출, build gate 없음") 이 reflow 규칙의 대상이
아니다 — 4개 중 가변인 건 `Editor` 하나뿐이다.

### 2.9.3 배경 — 정적, 앰비언트 요소 없음

- **wave·bubble 둘 다 두지 않는다** — §2.7(title)의 앰비언트 배경은
  최초 진입의 인상을 위한 것이고, mode-select는 song-select·settings·
  credits에서 Back으로 계속 돌아오는 허브다(D-2026-052의 통일 Back
  키가 세 화면 모두 여기로 보낸다). 매번 재생되는 배경 애니메이션은
  반복 방문에서 특별함보다 피로를 만들 위험이 크다고 판단해 배제했다.
  title·mode-select가 같은 공용 root([[scene]] §1)라는 점에서 시각적
  연속성을 고려했으나, 방문 빈도 차이가 더 중요한 기준으로 판단됐다.
- 배경 `--bg`, 텍스트 `--text`/`--cyan` — §1 기존 토큰. 새 색 없음.
