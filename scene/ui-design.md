...
├─ 정렬 · 레벨 ─────┬─ 그룹 · 없음 ────────────────────┤
├─ 정보 패널 (4fr) ─┬─ 목록 (6fr) ─────────────────────┤
│ 자켓(대, 패널 높  │ [폴더] 전체 128곡 · 42/128 CLEAR  │
│  이 기준)         │  ▸ row: 자켓 제목/아티스트 12345  │
│ 제목/아티스트     │  ▸ row: ...                       │
│ BPM · 길이        │                                    │
│ 2×2 기록 격자     │                                    │
├───────────────────┴────────────────────────────────────┤
│  ↑↓ 곡 이동   ←→ 난이도   Space 빠른 옵션   any 검색   │
└───────────────────────────────────────────────────────┘
```
...
- category 탭은 pill 형태(테두리 있는 캡슐, 단순 텍스트 라벨 아님). 지금은
  ...
  `--cyan` 밑줄/배경.
- 검색은 우측, 3-상태:
  - idle: 아이콘(`--dim`) + 힌트 텍스트 `아무 키나 입력`. `검색` 같은
    일반 라벨을 쓰지 않는다.
  - 입력 중: 실제 입력된 검색어 + 매치 수(`fo · 2개` 형식). 아이콘·텍스트
    `--cyan`.
  - 결과 없음: 입력 중과 같은 배치지만 매치 수와 아이콘이 `--j-miss`로
    바뀐다(기존 실패 상태 색을 attention 색으로 겸용 — 새 토큰 아님). 이때
    목록 영역은 row 대신 가운데 정렬된 안내(아이콘 + `검색 결과가 없습니다`)로
    바뀐다. [[song-select]] §6의 "결과 없음"이 이 표시를 가리킨다.
...
- `정렬 · {sortKey 표시명}` — 클릭하면 목록 옵션 overlay를 연다
  ([[song-select]] §8).
- `그룹 · {groupBy 표시명, 없으면 "없음"}` — 같은 overlay의 다른 탭/섹션을 연다.
...
- 검색하지 않을 때: folder 헤더가 count + 클리어 진척을 보여준다
  (`전체 128곡` / `42/128 CLEAR`, [[song-select]] §4). 접힌 folder는
  헤브론(`▸`) + 헤더 줄만 표시한다.
...
상시 표시 키 힌트 한 줄: `↑↓ 곡 이동`, `←→ 난이도`, `Space 빠른 옵션`,
`문자 입력 검색`. [[song-select]] §7/§13의 키 목록을 그대로 옮긴 표시이며
새 키 규칙이 아니다.
