...
status: Accepted (D-2026-051) — §6(scene.md §9 필드 추가)은 D-2026-054, §7-3(티어 색 대 실패 적색)은 D-2026-055로 닫혔다. §7-2(티어 색 대 Shape 색)는 결정이 아니라 scene 축 분리 확인으로 해소— 별도 D-log 없음. §2.5(곡 선택 레이아웃)는 D-2026-0xx(M3.5-1)로 확정 — 미해결 항목은 §2.5.7
...
1. **[게이트 확장 필요]** [[song-select]] §14 잔여의 "목록 옵션 overlay
   ...
   게이트 쪽에 요구사항만 남긴다.
2. **[디자인 공백 — 방향 필요]** 빈 library(곡을 아직 하나도 가져오지
   않은 상태)의 레이아웃이 이번 리뷰 목업에 없었다. [[song-select]]
   §11은 "안내 문구 + import 진입점"을 요구하지만 배치는 정해지지
   않았다. 이 절을 확정 짓기 전에 별도로 방향을 확인해야 한다.
