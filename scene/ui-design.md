...
2. **[백로그]** `prefers-reduced-motion` 대응 — §5(result 접근성)가
   "등장 연출은 `prefers-reduced-motion` 대응"이라 명시했던 것과 같은
   원칙이 title의 pulse·wave·bubble에도 적용돼야 하는지 이 리뷰에서
   다루지 않았다. §5가 이미 있는 프로젝트 규칙이라는 점에서 title도
   같은 처리가 필요할 가능성이 높지만, 정지 시 대체 표현(애니메이션
   끄기만 할지, 정적 배경으로 대체할지)은 제품 판단이 필요해 여기서
   임의로 정하지 않는다.
