...
- [x] lane symmetry 3-그룹(연속) — **닫힘**(M6-후속): `LineNum`이 1|2|3뿐이라 "3개 선택"은 항상 {1,2,3} 하나(다른 3-조합은 이 데이터 모델에 없다). 축 = 가운데(line2)의 현재 위치 그 자체(안 움직인다), line3=클릭 위치, line1=대칭 생성. 이 축은 수동 조절 대상이 아니다. **4개 이상·비연속은 결정 필요 항목이 아니라 구조적으로 성립하지 않는다**(D-2026-122 재정리) — `LineNum`이 1|2|3인 건 게임이 4레인·구분선 3개로 고정돼 있기 때문이라 "4번째 구분선"이 놓일 자리 자체가 없다. lane 수를 바꾸는 훨씬 큰 변경이 실제로 생긴다면 그때 새로 검토할 자리다.
- [x] 같은 dest tick·같은 체인 배치 충돌 = easing 갱신(shape·lane) — **닫힘**(D-2026-122): 원본 `addShapeEvt`의 "sameTickSameSide"(그 자리에서 easing만 갱신)를 재현했다. `targetPos`는 덮지 않는다. "한 클릭 = 한 undo"(`editor-commands.md` §6)를 지키기 위해 `updateShapeEasingCommand`/`updateLaneEasingCommand`가 add·update 목록을 함께 받아 한 커맨드로 낸다 — symmetry·그룹 배치처럼 한 클릭이 여러 체인을 건드릴 때 일부는 새로 놓이고 일부는 갱신돼도 커맨드 하나다. 이름은 동적으로 정해진다: 뭔가 새로 놓였으면 `AddShapeEvents`/`AddLaneEvents`, 전부 갱신뿐이면 `UpdateShapeEasing`/`UpdateLaneEasing`. `MutateShapeEvents`(드래그-end 전용)와는 트리거·필드가 달라 재사용하지 않는다.
...
- [x] Ctrl+D 구간 복제(shape·lane) — **닫힘**(D-2026-120): §7 "선택 구간을 그 길이만큼 바로 뒤에 복제(Ableton식)"를 `editor-commands.md` §6("기존 Add* command를 재사용한다")대로 구현했다. 구간 = 선택 전체의 실제 폭(가장 이른 실제 `startTick`~가장 늦은 dest tick) — 단일 선택은 그 자신의 길이만큼만 뒤로 옮겨진다. 서브모드 필터를 따른다(clipboard와 같은 기본값, mirror만 예외). 충돌(같은 dest tick·같은 체인)은 조용히 스킵 — "전부 충돌이면 toast"는 toast UI 자체가 없어(D-2026-114 선례) 여전히 구현하지 않는다.
- [x] Ctrl+D 구간 복제(notes 탭) — **닫힘**(D-2026-121): §7은 tab-scope를 가리지 않는 일반 규칙이라 notes 탭에도 적용했다. note·textEvent는 chain이 아니라 `startTick`이 곧 실제 배치 위치라(shapes/lane처럼 정규화가 다시 세우지 않는다) `copySelection`의 relTick(선택 최소 startTick 기준)을 그대로 구간 계산에 재사용할 수 있었다 — shapes에서 겪은 "단일 선택 구간이 0으로 계산돼 제자리에서 스스로와 충돌하는" 문제가 이 데이터 모델에서는 애초에 생기지 않는다. §1 "선택에 textEvents가 포함되면 함께 복사·붙여넣기"를 따라 note·text를 하나의 구간으로 합쳐 계산하되, dispatch는 기존 관례대로 각각 별도다(`AddNotes`/`AddTextEvents`). 충돌(같은 lane+tick+isWide)은 note만 조용히 스킵 — text는 원래도 충돌 검사가 없다.

M5-4가 결정 필요 항목으로 남겼던 6개 단순화 지점(Ctrl+F mirror·클립보드·symmetry 축 수동 조절·lane 그룹 토글·`laneGridDivisor`/`V` UI·Ctrl+D)이 D-2026-112/114/115/119/120으로 전부 닫혔고, 그 뒤에 남아 있던 나머지 두 항목(같은 dest tick 충돌 처리·lane symmetry 4개 이상)도 D-2026-122로 닫혔거나 구조적으로 성립하지 않는 것으로 재정리됐다 — shapes/lane 서브모드에 더 이상 "잔여" 항목이 없다.
