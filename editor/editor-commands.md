...
dispatch/undo/redo마다 listener가 발화한다. active scene redraw와 autosave schedule([[persistence]] §3)은 edit layer 조립부가 구독한다. command layer는 render/storage를 모른다.
...
“새 난이도”는 현재 chart에서 **새 독립 chart를 파생하여 session을 교체**한다.

- 같은 `songId` 유지;
- 유효한 새 `chartId`·difficulty 지정;
- metadata·timing·asset 연결을 시작값으로 복사할 수 있음;
- notes/shapeEvents/laneEvents/textEvents는 백지;
...
- [x] 새 chart = 같은 songId의 독립 chart 파생 `[번복 반영]`
