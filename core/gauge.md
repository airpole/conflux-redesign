...
- **`gauge`** (`playState.gauge = {hardPct, normalPct}`, 각 0~100) — 생명력 막대. 두 게이지는 **모든 모드에서 병렬 누적**된다(단일 코드 경로 — cascade가 특별해지지 않게). 모드가 어느 값을 소비할지 정하고(§2 표·§4), `as`/`ap`/`fc`는 게이지를 소비하지 않는다(표시 100 고정). `[수정 — 구 단일 `gaugeValue`]` 필드 구조 → [[data-model]] §9, 근거 → [[rationale#gauge를 2값 병렬 + tier 단일 구조로 통일한 이유]].
- **`tier`** (`playState.tier` ∈ `as`/`ap`/`fc`/`hard`/`normal`) — gauge의 구성 값 하나로, 현재 살아있는 최고 단계 `[신규]`. 단일 모드에서는 모드 대응 값으로 고정되고 cascade에서는 강등을 따라 내려간다(§4). 강등이 비가역(래칫)이라 이 값 하나가 래칫까지 표현한다. 필드 → [[data-model]] §9. gauge 서술은 별도 묶음말(구안 "lock") 없이 두 게이지 값 + tier로만 한다.
...
곡 시작부터 **모든 티어를 동시에 평가**한다: `as`/`ap`/`fc`의 terminate 조건과 게이지 2종(`hard`/`normal`)을 병렬로 굴린다. 살아있는 **가장 높은 티어가 현재 state**.
...
- **result 게이지 막대는 최종 생존 티어 기준**으로 표시한다 `[번복 — 구안은 "항상 hard 값"]`: `as`/`ap`/`fc` 생존 종결 → 100 고정 + 해당 state 색(§2와 동일 규칙), `hard` 단계 종결 → hard 값·색, `normal` 단계 종결 → normal 값·색. 플레이 중 막대(아래 항목)와 result가 연속되고, 각 단일 모드의 result 표시와 동치 — "cascade로 C = normal 모드 클리어"가 표시까지 성립한다. 근거 → [[rationale#cascade result 막대를 최종 티어 기준으로 바꾼 이유]].
- 현재 생존 티어는 §1의 `tier`로 추적한다.
- **탈락은 래칫(비가역)** — hard 게이지가 한 번 0에 닿으면 이후 delta로 값이 회복돼도 `H`로 복귀하지 않는다(`as`/`ap`/`fc`의 조건 위반이 비가역인 것과 같다). "cascade로 `H` = hard 모드로 클리어"의 동치를 지키기 위함 — hard 모드였다면 0에 닿은 순간 죽은 판이다. 근거 → [[rationale#cascade의 hard 탈락을 래칫으로 둔 이유]].
- **플레이 중 게이지 막대는 살아있는 최고 티어**를 따른다: `as`/`ap`/`fc` 생존 중엔 100 고정 + 해당 state 색, hard 단계면 hard 값·색, normal 단계면 normal 값·색 → [[theme]].

### 검증 시나리오

cascade는 [수정]이라 대조할 구 구현이 없다. 재구현 검증은 아래 표가 기준이다 — 같은 판정 시퀀스를 각 단일 모드로 쳤을 때와 **동일한 결과**여야 한다.

| # | 판정 시퀀스 (요약) | 티어 추이 | 최종 state |
|---|---|---|---|
...
[수정] — 구 코드 cascade는 `as`/`ap`/`fc` 티어만 한 칸 내리고 게이지 본체는 단일·연속(`as → ap → fc → bare gauge`)이었다. 이를 **게이지 2종(hard·normal)을 병렬 평가하는 최고-상태 모델**로 재정의. 근거 → [[rationale#gaugeMode를 단일 축 6종으로 둔 이유]].
