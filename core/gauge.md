...
|---|---|---|
| 1 | `forceEnded` | `F` |
| 2 | 판정된 단위 < chart의 판정 단위 총수 | `F` |
| 3 | MISS·GOOD·PERFECT 모두 0 | `AS` |
| 4 | MISS·GOOD 0 | `AP` |
| 5 | MISS 0 | `FC` |
| 6 | `tier` == `hard` | `H` |
| 7 | `normalPct` ≥ `NORMAL_CLEAR_PCT` | `C` |
| 8 | 그 외 | `F` |

- 2번 줄이 **완주 조건**이다 `[수정]` — `F`를 뺀 모든 마크는 곡의 모든 판정 단위가 판정된 판에만 붙는다. 마크의 뜻이 "이 성적으로 곡을 통과했다"이므로, 판정되지 않은 단위가 남아 있으면 그 뜻이 성립하지 않는다. 실제 판은 만료 처리가 끝을 쓸고 지나가 늘 완주 상태로 끝나므로 정상 경로에서는 이 줄에 걸리지 않는다. 구 코드는 이 조건이 없어 24단위 중 10단위만 판정된 판도 미스가 없으면 `AS`를 냈다 (설계 대장 GA-9).
- 6번 줄이 `H`와 `C`를 가르는 유일한 자리다. `normal` 모드는 `tier`가 `normal`이라 7·8로, `hard` 모드는 `tier`가 `hard`라 6으로 떨어지고, `as`/`ap`/`fc` 모드는 생존 자체가 MISS 0을 뜻해 3~5에서 끝난다. **cascade만 `tier`가 실제로 움직인다** — 그래서 cascade가 별도 산출 경로를 갖지 않는다.
- 노트가 하나도 없는 chart는 판정 단위 총수가 0이라 2번 줄을 통과하고 3번 줄에서 `AS`다 `[보존]`.
